...
import portofolio from "../../public/portofolio.pdf";
...
const Navbar = () => {
  ...
  return (
    <div className={``} id="about">
      ...
      <div
        className={`fixed w-full pt-5 pb-8 leading-9 text-center bg-color-four shadow-md ${
          isOpen ? "top-16" : "-top-64"
        } lg:-top-2 lg:shadow-none lg:z-30 lg:left-96 lg:ml-44 lg:pb-0 text-color-one transition-all ease-in-out duration-500 z-10 dark:bg-color-one dark:text-color-four`}
      >
        <ul className="lg:flex lg:gap-10 lg:items-center">
          ...
          <button
            className="px-5 py-2 mt-2 text-lg font-bold rounded-full lg:mt-0 bg-color-tree text-color-four hover:bg-[#308a8e]"
          ...
          <dialog id="my_modal_9" className="modal">
            <form
              method="dialog"
              className="modal-box bg-color-four dark:bg-color-one"
            >
              <button className="absolute shadow-sm bg-gray-50 dark:bg-gray-700 dark:hover:bg-gray-900 btn btn-sm btn-circle btn-ghost right-2 top-2">
                ✕
              </button>
              <a href={portofolio} target="_blank">
                <p className="w-full py-2 mt-5 text-white rounded-md bg-color-tree hover:bg-cyan-600">
                  Download CV
                </p>
              </a>
              <a href="mailto:ikhwanulhusna111@gmail.com" target="_blank">
                <p className="w-full py-2 mt-2 text-white rounded-md bg-color-tree hover:bg-cyan-600">
                  Send Email
                </p>
              </a>
            </form>
        ...
        <div
          className={`hidden lg:block h-1 -mt-2 dark:bg-color-four bg-color-one transition-all duration-500 ease-in-out ${
            activeLink === 1 && "w-[60px]"
          } ${activeLink === 2 && "translate-x-[100px] w-[46px]"} ${
            activeLink === 3 && "w-[95px] translate-x-[188px]"
          } ${activeLink === 4 && "w-[72px] translate-x-[323px]"}`}
          id="activeLink"
        ></div>
      </div>
    </div>
